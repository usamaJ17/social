...
function getproject_Gallery($project_id){
    $project_details = DB::table('bc_projects')->where('id', $project_id)->first();
    // return $project_details;
        if (empty($project_details->project_photos)){
            return $project_details->project_photos;
        }
        ...
        $items = explode(",", $project_details->project_photos);
        foreach ($items as $k => $item) {
			
			if( !\Modules\Media\Helpers\FileHelper::hasImage($item) ){
				continue;
			}
			
            if($item) {
                $large = \Modules\Media\Helpers\FileHelper::url($item, 'full');                				
				$medium = \Modules\Media\Helpers\FileHelper::url($item, 'medium');
                $thumb = \Modules\Media\Helpers\FileHelper::url($item, 'thumb');
                $list_item[] = [
                    'large' => $large,                    					
					'medium' => $medium,
                    'thumb' => $thumb,
...
$check_user_type = DB::table('core_model_has_roles')->where('model_id',Auth::id())->first();
    if (!empty($check_user_type) && $check_user_type->role_id == 2) {
        return 'customer';
    }

    if (!empty($check_user_type) && $check_user_type->role_id == 1) {
        return 'contractor';
    }
...
//bc property category
function bc_property_category($bc_property_id){
$bc_property_category = DB::table('bc_property_category')
->where('bc_property_category_relationships.property_id', $bc_property_id)
->orderBy('bc_property_category_relationships.id','asc')
->join('bc_property_category_relationships', 'bc_property_category_relationships.category_id' , '=', 'bc_property_category.id')
->first();

    if (!empty($bc_property_category)) {
        return $bc_property_category->name;
    }else{
        return null;
    }
}
...
//create_user
function get_bc_properties(){
$bc_properties = DB::table('bc_properties')->where('create_user',Auth::id())->first();
    if (!empty($bc_properties)) {
        return $bc_properties->id;
    }else{
        return 0;
    }
}
...
//get img id
function get_bc_img_id(){
$bc_properties = DB::table('bc_properties')->where('create_user',Auth::id())->first();
    if (!empty($bc_properties)) {
        return $bc_properties->image_id;
    }else{
        return 0;
    }
}
...
//get_ch_location
function get_ch_location($id){
$get_ch_location = DB::table('bc_locations')->where('id', $id)->first();
    if (!empty($get_ch_location)) {
        return $get_ch_location;
    }else{
        return 'Null';
    }
}
...
//get_ch_category
function get_ch_category($id){
$get_ch_category = DB::table('bc_property_category')->where('id', $id)->first();
    if (!empty($get_ch_category)) {
        return $get_ch_category;
    }else{
        return 'Null';
    }
}
...
//get_ch_type
function get_ch_type($id){
$get_ch_type = DB::table('bc_terms')->where('id', $id)->first();
    if (!empty($get_ch_type)) {
        return $get_ch_type;
    }else{
        return 'Null';
    }
...
//get_ch_budget
function get_ch_budget($id){
$get_ch_budget = DB::table('bc_terms')->where('id', $id)->first();
    if (!empty($get_ch_budget)) {
        return $get_ch_budget;
    }else{
        return 'Null';
    }
}
...
//get_ch_language
function get_ch_language($id){
$get_ch_language = DB::table('bc_terms')->where('id', $id)->first();
    if (!empty($get_ch_language)) {
        return $get_ch_language;
    }else{
        return 'Null';
    }
}
...
function get__title(){
    if (check_user_type() == 'contractor') {
        $bc_properties = DB::table('bc_properties')->where('create_user',Auth::id())->first();
        if (!empty($bc_properties)) {
        return $bc_properties->title;
        }else{
        return 'Demo contractor';
        }
    }else{
...
//get one image of project
function project_oneImg($img_id){
	
	$project_oneImg = DB::table('media_files')->where('id', $img_id)->first();
	
	if (!empty($project_oneImg)) {
		
        $path_info = pathinfo($project_oneImg->file_path);		
  		$file_path = $path_info['dirname'] . '/' . $path_info['filename'] . '-600.' . $path_info['extension']; 						
		$w_file_path = $path_info['dirname'] . '/' . $path_info['filename'] . '.' . $path_info['extension']; 						
		
		if (check_file_exists_here(asset('uploads/'.$file_path))) {			
			return asset('uploads/'.$file_path);
        }
elseif(check_file_exists_here(asset('uploads/'.$w_file_path))){
    
    
    	return asset('uploads/'.$w_file_path);
}        
        
        else{
            return url('/').'/public/uploads/0000/44/2022/11/22/dimg.jpg';
        }
	}else{
        return 'https://cdn.pixabay.com/photo/2017/03/28/12/10/chairs-2181947_1280.jpg';
    }
}

//get contractor_photo
function contractor_photo($img_id){
	
	$get_contractor_photo = DB::table('media_files')->where('id', $img_id)->first();
	
	if (!empty($get_contractor_photo)) {
		$path_info = pathinfo($get_contractor_photo->file_path);
		$file_path = $path_info['dirname'] . '/' . $path_info['filename'] . '-600.' . $path_info['extension'];
...
//get contractor_icon_photo
function contractor_profile_icon_photo($img_id){
	
	$get_contractor_photo = DB::table('media_files')->where('id', $img_id)->first();
	
	if (!empty($get_contractor_photo)) {
		$path_info = pathinfo($get_contractor_photo->file_path);
		$file_path = $path_info['dirname'] . '/' . $path_info['filename'] . '-150.' . $path_info['extension'];

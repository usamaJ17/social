<section class="home-one home1-overlay bg-img1" style="background-image:url({{$bg_image_url}})">
    <div class="container">
        <div class="row posr">
            <div class="col-lg-12">
                <div class="home_content {{!empty($show_quantity_list) ? 'home3' : 'home1'}}">
                    <div class="home-text text-center {{!empty($show_quantity_list) ? 'home3' : 'home1'}}">
                        @if(!empty($title))
                        <h2 class="fz50 mx-lg-5">{{$title}}</h2>
                        @endif
                        @if(!empty($sub_title))
                    ...
                    <div class="row justify-content-center">
                        <div class="col-lg-10 col-xl-8">
                            <div class="home_adv_srch_opt text-center {{!empty($show_quantity_list) ? 'home3 pt20' : ''}}">
                                <div class="wrapper">
                                    <div class="home_adv_srch_form {{!empty($style) ? $style : 'home1'}}">
                                                    ...
                                                    @if(!empty($service_types))
                                                        ...
                                                        @foreach ($service_types as $service_type)
                                                            ...
                                                            <div role="tabpanel" class="tab-pane @if($number == 0) active @endif" id="bc_{{$service_type}}">
                                                                @include(ucfirst($service_type).'::frontend.layouts.search.form-search',['style'=>$style])
                                                            </div>
                                                            @php $number++; @endphp
                                                        @endforeach
                                                    @endif
                    ...
                    <div class="row">
                        @if(!empty($show_counter_to) and  !empty($counter_to_list) && count($counter_to_list) > 0)
                            @includeIf('Template::frontend.blocks.form-search-all-service.elements.counter-to-number')
                        @endif
                    </div>

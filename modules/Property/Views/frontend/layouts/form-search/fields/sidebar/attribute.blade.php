            ...
            <div class="wrapper">
                ...
                <ul>
                    @foreach($attr->terms as $term)
                        @php($idLabel = 'customCheck'.Str::random(5))
                    <li>
                        <div class="custom-control custom-checkbox">
                            <input type="checkbox" class="custom-control-input" name="terms[]" value="{{$term->id}}" id="{{$idLabel}}"
                               @if(!empty(Request::input('terms')))
                                ...
                                @endif
                            >
                            <label class="custom-control-label" for="{{$idLabel}}">{{$term->name}}</label>
                        </div>
                    </li>
                    @endforeach
                </ul>

...
class MediaController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $folder = $folder . date('Y/m/d');		$nm = substr($file->getClientOriginalName(), 0, strrpos($file->getClientOriginalName(), '.'));		
        $newFileName = Str::slug("{$nm}");
        ...
        if ($check) {
            try {
				...
				$fileObj->media_for = $request->has("prefixed") ? $request->get("prefixed") : "" ;
                $fileObj->file_size = $file->getSize();
                $fileObj->file_type = $file->getMimeType();
                $fileObj->file_extension = $file->getClientOriginalExtension();
                
				if( $request->has("prefixed") && !empty($request->get("prefixed")) ){
					
					$arr = explode("@@", $request->get("prefixed"));
					
					if(count($arr)>1){
						if($arr[0]=="by_contractor"){
							$fileObj->create_user = $arr[1] ;
						}else{
                ...
                $fileObj->sizes = [
                    ...
                    '900'     => url('media/preview/'.$fileObj->id .'/medium'),
    ...
    public function getLists(Request $request)
    {
		...
		$request->has("prefixed") ? $request->get("prefixed") : "" ;
		...
		if( $request->has("prefixed") && !empty($request->get("prefixed")) ){
			...
			if(count($arr)>1){
				if($arr[0]=="by_contractor"){
					$model->where('create_user',  $arr[1] );
					$model2->where('create_user',  $arr[1] );
				}else{
					$model->where('media_for',  $request->get("prefixed") );
					$model2->where('media_for',  $request->get("prefixed") );	

...
class ReviewController extends Controller
{
    ...
    public function addReview(Request $request,$is_return = false)
    {
        ...
        $rate = $request->input('review_rate');
        if ($rate < 1) {
            return redirect()->to(url()->previous() . '#bc-reviews')->with('error', 'select rating!');
        }
		
		$check_review = DB::table('bc_review')->where('object_id', $service_id)->whereNull('deleted_at')->where('vendor_id', Auth::id())->first();		 
        
		if (!empty($check_review)) {
            return redirect()->to(url()->previous() . '#bc-reviews')->with('error', 'Already reviewed error');
        }
	...
	public function deleteReview(Request $request){
		...
		$review = Review::where('id',$request->id)->where('vendor_id',Auth::id())->first();
		
		if (empty($review) ) {
             return redirect()->to(url()->previous())->with('error', 'Review not found');
        }
		
		$dlt_review = Review::where('id', $request->id)->delete();
		$dlt_meta_review = DB::table('bc_review_meta')->where('review_id', $review->id)->delete();
        
		if ($dlt_review && $dlt_meta_review) {
            return redirect()->to(url()->previous())->with('success', __('Review deleted successfully!') );
        }
        
		return redirect()->to(url()->previous())->with('error', __('Something wrong!') );
	}
}
